...
class TextureBakery:
    ...
    def bake_grid():
        ...
        size = 512
        # BRIGHTER BACKGROUND: Deep Purple (No longer "Black")
        img = Image.new('RGB', (size, size), (60, 30, 80)) 
        ...
        for i in range(0, size, step):
            # Horizontal
            draw.line([(0, i), (size, i)], fill=(0, 240, 255), width=3)
            # Vertical
            draw.line([(i, 0), (i, size)], fill=(0, 240, 255), width=3)
        ...
        for _ in range(3000):
            ...
            # Brighter sparkles
            pixels[x, y] = (100, 80, 150)

        return Texture(img)

    @staticmethod
    def bake_plasma_noise(r, g, b):
        """Generates a smooth plasma cloud for ink WITH COLOR BAKED IN"""
        size = 128
        ...
        for x in range(size):
            for y in range(size):
                ...
                # Soft alpha gradient
                alpha_val = int(((v + 3) / 6) * 255)
                alpha_val = max(100, min(240, alpha_val))
                
                # BAKE THE COLOR DIRECTLY INTO THE PIXEL
    ...
    def bake_brushed_metal():
        ...
        size = 256
        # Light Grey Base
        img = Image.new('RGB', (size, size), (180, 185, 190))
        draw = ImageDraw.Draw(img)
        ...
        for _ in range(500):
            ...
            c = random.randint(160, 210)
            draw.line([(x, y), (x+l, y)], fill=(c, c, c+10), width=1)
            
        # Panel border
        draw.rectangle([0,0,size-1,size-1], outline=(120,120,130), width=3)
...
PALETTES = [
    {'ink': (0, 255, 255), 'dark': color.rgb(0, 100, 100)},   # Cyan Tuple
    {'ink': (255, 50, 255), 'dark': color.rgb(100, 20, 100)}, # Magenta Tuple
    {'ink': (50, 255, 50), 'dark': color.rgb(20, 100, 20)},   # Green Tuple
    {'ink': (255, 180, 0), 'dark': color.rgb(100, 70, 0)},    # Amber Tuple
]
ENEMY_COLOR_TUPLE = (255, 50, 80) # Hot Red
ENEMY_COLOR = color.rgb(255, 50, 80)
...
class UltraSplatoon:
    ...
    def rebake_ink(self):
        """Bakes ink textures with current palette colors"""
        p_col = PALETTES[self.palette_index]['ink']
        # Bake Player Ink (Cyan/Magenta/etc)
        self.tex_ink_player = TextureBakery.bake_plasma_noise(p_col[0], p_col[1], p_col[2])
        # Bake Enemy Ink (Red)
        self.tex_ink_enemy = TextureBakery.bake_plasma_noise(ENEMY_COLOR_TUPLE[0], ENEMY_COLOR_TUPLE[1], ENEMY_COLOR_TUPLE[2])
    ...
    def create_menu(self):
        """Create main menu"""
        p_col = PALETTES[0]['ink']
        title_color = color.rgb(p_col[0], p_col[1], p_col[2])
    ...
    def create_hud(self):
        """Create in-game HUD"""
        p_col = PALETTES[self.palette_index]['ink']
        hud_color = color.rgb(p_col[0], p_col[1], p_col[2])
        ...
        self.hud_elements['ink_fill'] = Entity(parent=camera.ui, model='quad', color=hud_color, scale=(0.3, 0.015), position=(-0.55, 0.42), origin=(-0.5, 0))
    ...
    def spawn_ink_decal(self, x, z, team, size=2):
        ...
        # SELECT PRE-BAKED TEXTURE
        tex = self.tex_ink_player if team == TEAM_PLAYER else self.tex_ink_enemy
        ...
        decal = Entity(
            ...
            color=color.white, # Texture provides the color now!
            texture=tex, 
            alpha=0.9
        )
        decal.team = team
    ...
    def input(self, key):
        if self.state == 'menu':
            ...
            if key == 'r': 
                ...
                # Re-bake texture and update HUD color
                self.rebake_ink()
                p_col = PALETTES[self.palette_index]['ink']
                hud_color = color.rgb(p_col[0], p_col[1], p_col[2])
                self.hud_elements['ink_fill'].color = hud_color
                self.show_toast("INK RE-BAKED!")
