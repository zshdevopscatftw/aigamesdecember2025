...
SAVE_FILE = os.path.join(SCRIPT_DIR, "ultra_tetris_save.txt")
...
def save_game(save_data: SaveData):
    ...
    try:
        ...
        with open(SAVE_FILE, 'w') as f:
            f.write("# ULTRA!TETRIS Save File\n")
            f.write(f"# Last saved: {save_data.last_played}\n")
            f.write("# Do not edit manually!\n")
            f.write("\n")
            f.write("# === SETTINGS ===\n")
            f.write(f"music_volume={save_data.music_volume}\n")
            f.write(f"sfx_volume={save_data.sfx_volume}\n")
            f.write(f"music_enabled={save_data.music_enabled}\n")
            f.write(f"sfx_enabled={save_data.sfx_enabled}\n")
            f.write("\n")
            f.write("# === STATISTICS ===\n")
            f.write(f"total_games={save_data.total_games}\n")
            f.write(f"total_lines={save_data.total_lines}\n")
            f.write(f"total_tetrises={save_data.total_tetrises}\n")
            f.write(f"total_score={save_data.total_score}\n")
            f.write(f"high_score={save_data.high_score}\n")
            f.write(f"max_level={save_data.max_level}\n")
            f.write(f"total_holds={save_data.total_holds}\n")
            f.write(f"total_hard_drops={save_data.total_hard_drops}\n")
            f.write(f"play_time_seconds={save_data.play_time_seconds}\n")
            f.write("\n")
            f.write("# === TROPHIES ===\n")
            f.write(f"unlocked_trophies=[{','.join(save_data.unlocked_trophies)}]\n")
            f.write(f"trophy_timestamps={json.dumps(save_data.trophy_timestamps)}\n")
            f.write("\n")
            f.write(f"last_played=\"{save_data.last_played}\"\n")
...
def draw_piece_preview(surface, shape: str, x: int, y: int, size: int = CELL_SIZE):
    ...
    offset_x = x + (4 - width) * size // 2
    offset_y = y + (4 - height) * size // 2
    
    for cx, cy in rotations:
        px = offset_x + (cx - min_x) * size
        py = offset_y + (cy - min_y) * size
...
def draw_credits(surface):
    ...
    lines = [
        ("ULTRA!TETRIS", font_title, NES_CYAN),
        ("PYGAME PORT", font_medium, NES_GREEN),
        ("", None, None),
        ("DEVELOPED BY", font_medium, NES_WHITE),
        ("TEAM FLAMES", font_large, NES_YELLOW),
        ("", None, None),
        ("PUBLISHED BY", font_medium, NES_WHITE),
        ("SAMSOFT", font_large, NES_GREEN),
        ("", None, None),
        ("© THE TETRIS COMPANY", font_medium, NES_RED),
        ("TETRIS ® & © 1985-2025", font_small, NES_GRAY),
        ("ALL RIGHTS RESERVED", font_small, NES_GRAY),
        ("", None, None),
        ("ORIGINAL GAME BY", font_small, NES_WHITE),
        ("ALEXEY PAJITNOV", font_medium, NES_PURPLE),
        ("", None, None),
        ("PRESS ESC TO RETURN", font_small, NES_GRAY),
    ]
    ...
    for text, fnt, color in lines:
        ...
        y += 20 if fnt == font_small else 28
